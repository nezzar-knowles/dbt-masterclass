...
select 'users' as table_name, count(*) as row_count from {{ source('thelook_ecommerce', 'users') }}
union all
select 'orders', count(*) from {{ source('thelook_ecommerce', 'orders') }}
union all
select 'order_items', count(*) from {{ source('thelook_ecommerce', 'order_items') }}
union all
select 'products', count(*) from {{ source('thelook_ecommerce', 'products') }}
union all
select 'inventory_items', count(*) from {{ source('thelook_ecommerce', 'inventory_items') }}
union all
select 'distribution_centers', count(*) from {{ source('thelook_ecommerce', 'distribution_centers') }}
union all
select 'events', count(*) from {{ source('thelook_ecommerce', 'events') }};
...
select distinct status from {{ source('thelook_ecommerce', 'orders') }};
select distinct status from {{ source('thelook_ecommerce', 'order_items') }};
select distinct event_type from {{ source('thelook_ecommerce', 'events') }};
select distinct traffic_source from {{ source('thelook_ecommerce', 'users') }};
