...
Show them the dbt_project.yml, explain the default (including target) is showing and then set some materialization and schema.
...
Run them through the dbt cloud interface

Module 2
Create sources.yml and define the necessary variables. Save it under the staging layer.
...
Add comments using the single line and multi line 
...
Multi line description using > in yaml
Run dbt snapshot
...
Write a singular test that will output error. Run dbt tests. Show them how store_failures can show you the rows that fail. Show how you can set it on a column level or global level.
...
Write a unit test and run it using `dbt test --select test_type:unit`
...
Do dbt docs generate then view documentation.
